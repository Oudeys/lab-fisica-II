...
% Pacchetti per la gestione dei caratteri e del layout
\usepackage[T1]{fontenc}         % Codifica dei caratteri per una migliore gestione degli accenti
\usepackage{lmodern}             % Font moderni
\usepackage{microtype}           % Miglioramento della tipografia
...
\usepackage{hyperref}            % Per collegamenti ipertestuali senza bordi
\usepackage{xcolor}              % Per il supporto ai colori
\usepackage{todonotes}           % Per aggiungere note durante la scrittura
...
\pagestyle{fancy}
\fancyhead[C]{\small Seconda Esercitazione di Laboratorio} % Intestazione generale centrata
